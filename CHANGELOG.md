...
- The installer and the manual instructions now put the loader and the mod
  ...
  package `Content` folder, and so do `CameraUnlock.ini` and
  `HeadTracking.log`.
...
- Settings move to `CameraUnlock.ini`, beside `KingdomCome.exe`: `Bin\Win64MasterMasterSteamPGO\CameraUnlock.ini` on Steam and GOG, `CameraUnlock.ini` in the package `Content` folder on Xbox Game Pass. Earlier versions of the mod kept these settings in `HeadTracking.ini`, in the same folder. The first time this version starts and finds no `CameraUnlock.ini`, it reads your settings from `HeadTracking.ini` and writes them into `CameraUnlock.ini`. It never changes `HeadTracking.ini`, and does not read it again while `CameraUnlock.ini` exists.
...
- The import carries every setting across under its new name:
...
- The tracking mode you pick with `Page Up` and the yaw mode you pick with
  `Page Down` are now saved to `CameraUnlock.ini` as they change, and come back
...
- `MoveCrosshair`, the setting that left the game's crosshair at screen
  centre. The crosshair now always follows your aim, and where
  `HeadTracking.ini` turned it off, the import logs that setting as not
  carried. If your file still has the long-retired `ShowReticle`, the import
  logs a warning that tells you to use `MoveCrosshair` instead. That advice is
  out of date: `MoveCrosshair` is gone too.
