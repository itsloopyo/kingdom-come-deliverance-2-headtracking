...
# Packaging for Kingdom Come: Deliverance II Head Tracking (C++ / CMake, no .csproj).
# Produces two ZIPs in release/:
#   - KingdomComeDeliverance2HeadTracking-v{version}-installer.zip (GitHub Release)
#   - KingdomComeDeliverance2HeadTracking-v{version}-nexus.zip     (Nexus, extract to game folder)
#
...
$modName    = 'KingdomComeDeliverance2HeadTracking'
...
# The installer ZIP is a binary distribution as much as the Nexus one, so a
# missing LICENSE or THIRD-PARTY-NOTICES.md is a licence violation, not a
# cosmetic gap. Throw rather than guarding with Test-Path - a silent skip turns
