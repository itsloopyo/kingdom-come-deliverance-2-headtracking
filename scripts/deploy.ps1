...
# Deploy the built KingdomComeDeliverance2HeadTracking.asi into the game's exe
...
# KINGDOM_COME_DELIVERANCE_2_PATH env var -> Steam registry / library folders ->
# games.json.
...
$asi = Join-Path $projectDir "build/$Configuration/KingdomComeDeliverance2HeadTracking.asi"
...
if (-not $GamePath) {
    ...
    $GamePath = Find-GamePath -GameId 'kingdom-come-deliverance-2'
}

if (-not $GamePath -or -not (Test-Path $GamePath)) {
    Write-Error "Could not locate Kingdom Come: Deliverance II. Set KINGDOM_COME_DELIVERANCE_2_PATH or pass the install path as the first argument."
    exit 1
}

$exeDir = Join-Path $GamePath 'Bin\Win64MasterMasterSteamPGO'
