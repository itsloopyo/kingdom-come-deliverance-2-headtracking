...
.DESCRIPTION
  ...
  layouts differ - Steam and GOG put the binaries under
  Bin\Win64MasterMasterSteamPGO, the Game Pass package puts them flat in Content
  - so the DLL is looked for beside the executable rather than at a fixed
  relative path.

  It checks WHGame.dll, NOT KingdomCome.exe: the exe is a 1.5 MB launcher stub,
  ...
  WHGameArm.dll ships beside it and is never a target: it is the ARM64 build of
  the same code, so its RVAs are unrelated.
...
if ($DllPath) {
    ...
    foreach ($install in Get-Kcd2Installs) {
        $dll = Join-Path $install.ExeDirectory 'WHGame.dll'
        if (-not (Test-Path -LiteralPath $dll)) {
            throw ("{0} install at {1} has no WHGame.dll beside the exe." -f $install.Store, $install.Path)
        }
...
if ($targets.Count -eq 0) {
    Write-Host "ERROR: Could not locate Kingdom Come: Deliverance II. Pass the WHGame.dll path positionally or set `$env:KINGDOM_COME_DELIVERANCE_2_PATH." -ForegroundColor Red
    exit 2
}
...
foreach ($target in $targets) {
    ...
    if ($running.TimeDateStamp -gt $newest.TimeDateStamp) {
        ...
        Write-Host "  The DLL is OLDER than the newest profile - let the store finish updating."
    } else {
        Write-Host "  Same build date, different size/checksum - a repacked or modified DLL."
    }
    Write-Host ""
    Write-Host ("  Paste-ready profile stub for src/builds/{0}_offsets.cpp:" -f $(if ($target.Store -eq 'Game Pass') { 'gdk' } else { 'steam' }))
    Write-Host ("    /* Name        */ `"{0}-win64-{1}`"," -f $(if ($target.Store -eq 'Game Pass') { 'gdk' } else { 'steam' }), ([DateTimeOffset]::FromUnixTimeSeconds($running.TimeDateStamp).UtcDateTime.ToString('yyyyMMdd')))
