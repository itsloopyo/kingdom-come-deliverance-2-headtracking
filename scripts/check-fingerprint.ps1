...
.DESCRIPTION
  ...
  It checks Bin\Win64MasterMasterSteamPGO\WHGame.dll, NOT KingdomCome.exe:
  the exe is a 1.5 MB launcher stub, and every address this mod pins is an RVA
  into WHGame.dll.
...
function Resolve-DllPath {
    ...
    $gameRoot = Find-GamePath -GameId 'kingdom-come-deliverance-2'
    if (-not $gameRoot) {
        throw "Could not locate Kingdom Come: Deliverance II. Pass the WHGame.dll path positionally or set `$env:KINGDOM_COME_DELIVERANCE_2_PATH."
    }
    $dll = Join-Path $gameRoot 'Bin\Win64MasterMasterSteamPGO\WHGame.dll'
